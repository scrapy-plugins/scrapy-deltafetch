...
dbmodule = None
try:
    dbmodule = import_module("bsddb3").db
except ModuleNotFoundError:
    pass


@skipIf(dbmodule is None, "bsddb3 is not found on the system")
class DeltaFetchTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.db_path = os.path.join(self.temp_dir, f"{self.spider.name}.db")
    ...
    def test_init_from_crawler(self):
        ...
        with mock.patch(
        ...
        ) as data_dir, mock.patch("scrapy.utils.project.inside_project") as in_project:
            ...
            assert isinstance(instance, self.mwcls)
            self.assertEqual(instance.dir, os.path.join(self.temp_dir, "deltafetch"))
            ...
            instance = self.mwcls.from_crawler(crawler)
            assert isinstance(instance, self.mwcls)
            self.assertEqual(instance.dir, os.path.join(self.temp_dir, "other"))
            self.assertEqual(instance.reset, True)
    ...
    def test_spider_opened_new(self):
        ...
        assert isinstance(mw.db, type(dbmodule.DB()))
        assert mw.db.items() == []
        assert mw.db.get_type() == dbmodule.DB_HASH
        assert mw.db.get_open_flags() == dbmodule.DB_CREATE
    ...
    def test_spider_opened_existing_spider_reset(self):
        ...
        mw.spider_opened(self.spider)
        assert mw.db.get_open_flags() == dbmodule.DB_TRUNCATE
    ...
    def test_spider_opened_reset_non_existing_db(self):
        ...
        assert (
            mw.db.get_open_flags() == dbmodule.DB_CREATE
            or mw.db.get_open_flags() == dbmodule.DB_TRUNCATE
        )
    ...
    def test_spider_closed(self):
        ...
        self.assertRaises(dbmodule.DBError, mw.db.fd)
    ...
    def test_process_spider_output(self):
        ...
        self.assertEqual(
            list(mw.process_spider_output(response, result, self.spider)), [result[0]]
        )
        ...
        self.assertEqual(set(mw.db.keys()), set([b"test_key_1", b"test_key_2"]))

        # if the spider returns items, the request's key is added in db
        result = [Item(), "not an item"]
        self.assertEqual(
            list(mw.process_spider_output(response, result, self.spider)), result
        )
        self.assertEqual(set(mw.db.keys()), set([b"key", b"test_key_1", b"test_key_2"]))
        assert mw.db[b"key"]
    ...
    def test_get_key(self):
        ...
        self.assertEqual(
            mw._get_key(test_req1), to_bytes(request_fingerprint(test_req1))
        )
    ...
    def _create_test_db(self):
        # !Cannot use context manager for db because there is no Attribute __enter___
        db = dbmodule.DB()
        # truncate test db if there were failed tests
        db.open(
            self.db_path, dbmodule.DB_HASH, dbmodule.DB_CREATE | dbmodule.DB_TRUNCATE,
        )
